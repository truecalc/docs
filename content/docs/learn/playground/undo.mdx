...
Every edit — typing a value, filling, pasting, clearing — is one undoable step. Press **Ctrl+Z** to revert the last step and **Ctrl+Y** (or **Ctrl+Shift+Z**) to re-apply it — or click the toolbar's **Undo**/**Redo** buttons, the first two controls, same as Google Sheets. Both grey out when there's nothing left to undo/redo. A fresh edit after undoing clears the redo history, exactly like Google Sheets.
