...
return a value. TrueCalc implements a wide range of Google Sheets
functions, each verified against the conformance fixtures — see the
[full function catalogue](/reference/functions) for the current count.
...
The complete function catalogue is generated into the
[Reference tab](/reference/functions) from the engine's registry.
